...
type FileArchive = {
    ArchivePath: string
} with member this.ToPulumiType = FileArchive(this.ArchivePath)

type RemoteArchive = {
    ArchiveUri: string
} with member this.ToPulumiType = RemoteArchive(this.ArchiveUri)

type RemoteAsset = {
    Uri: string
} with member this.ToPulumiType = RemoteAsset(this.Uri)

type FileAsset = {
    Path: string
} with member this.ToPulumiType = FileAsset(this.Path)

type StringAsset = {
    Text: string
} with member this.ToPulumiType = StringAsset(this.Text)
...
} with member this.ToPulumiType =
          this.Assets |>
          Map.map (fun _ a -> match a with
