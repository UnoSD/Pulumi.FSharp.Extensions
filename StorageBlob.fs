...
module StorageBlob =
    ...
    type StorageBlobBuilder internal () =
        ...
        member __.Name(args : StorageBlobArgsRecord, name) = { args with Name = name }
        
        [<CustomOperation("blobType")>]
        member __.Access(args, blobType) = { args with Type = blobType }
        
        [<CustomOperation("container")>]
        member __.Container(args, container) = { args with StorageContainer = Object container }
        
        member __.Container(args, containerName) = { args with StorageContainer = Name containerName }
       
        [<CustomOperation("source")>]
        member __.Source(args, source) = { args with Source = source }
        
        member __.Run (args : StorageBlobArgsRecord) =
             run args
