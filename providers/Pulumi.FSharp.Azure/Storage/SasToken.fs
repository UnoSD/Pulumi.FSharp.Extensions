...
type SasTokenBuilder internal () =
    ...
    let granularSas args range permissions =
        output {
            ...
            let! containerName = args.StorageContainer.Name
    ...
    member __.Run(args) =
        ...
        secretOutput { return! token }

    [<CustomOperation("blob")>]
    member __.Blob(args, blob) = { args with Blob = blob }
    ...
    member __.Duration(args, range) = { args with TimeRange = Some range }
