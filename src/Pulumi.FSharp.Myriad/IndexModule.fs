...
let createModules provider ((indexTypes, qualifiedTypes): PulumiModule list * PulumiModule list) =
    let groupSub contentList =
        ...
        |> List.map (fun (subName, content) -> {
            Name = subName
            Content =
                content
                |> Seq.collect (fun c -> c.Content)
        })
    ...
    let createSubmodule subName rootNamespace content =
        createModule'
            subName
            [
                $"{provider}.{rootNamespace}.{subName}"
                $"{provider}.Types.Inputs.{rootNamespace}.{subName}"
            ]
            content
    ...
    let createSubmodules rootNamespace =
        Seq.collect (
            ...
            | {
                  Content = content
                  SubNamespace.Name = None
              } -> content
            | {
                  Content = content
                  Name = Some subName
              } -> seq { createSubmodule subName rootNamespace content }
        )

    let qualifiedTypesModules =
        qualifiedTypes
        |> List.map (
            (fun qualifiedTypeModule ->
                (String.split '.' qualifiedTypeModule.ResourceProviderNamespace.Value,
            ...
            | [| rootNamespace; subNamespace |], content -> {
                Name = rootNamespace
        ...
        |> List.map (fun (rootNamespaceName, content) ->
            groupSub content
    ...
    let indexTypesAsts =
        indexTypes
        |> Seq.collect (fun x -> x.Content)

    let letCombineImplementation =
        let fromRcd =
            SynPat.CreateLongIdent(
                LongIdentWithDots.CreateString("_combine"),
                [
                    Pat.paren (
                        Pat.tuple (
                            ...
                            [
                                SynPat.CreateParen(
                                    SynPat.Tuple(
                                        ...
                                        [
                                            SynPat.CreateLongIdent(
                                                LongIdentWithDots.CreateString("rName"),
                                                []
                                            )
                                            SynPat.CreateLongIdent(
                                                LongIdentWithDots.CreateString("rArgs"),
                                                []
                                            )
                                        ],
                                        range.Zero
                                ...
                                SynPat.CreateParen(
                                    SynPat.Tuple(
                                        ...
                                        [
                                            SynPat.CreateLongIdent(
                                                LongIdentWithDots.CreateString("lName"),
                                                []
                                            )
                                            SynPat.CreateLongIdent(
                                                LongIdentWithDots.CreateString("lArgs"),
                                                []
                                            )
                                        ],
                                        range.Zero
        ...
        let matchExpr =
            Expr.paren (
                Expr.match' (
                    Expr.tuple (Expr.ident ("lName"), Expr.ident ("rName")),
                    [
                        Match.clause (Pat.tuple (Pat.null', Pat.null'), Expr.null')
                        Match.clause (
                            Pat.tuple (Pat.null', Pat.ident ("name")),
                            Expr.ident ("name")
                        )
                        Match.clause (
                            Pat.tuple (Pat.ident ("name"), Pat.null'),
                            Expr.ident ("name")
                        )
    ...
    let letCombineCrosImplementation =
        let fromRcd =
            SynPat.CreateLongIdent(
                LongIdentWithDots.CreateString("_combineCros"),
                [
                    Pat.paren (
                        Pat.tuple (
                            Pat.paren (Pat.tuple ("rName", "rArgs", "rCros")),
